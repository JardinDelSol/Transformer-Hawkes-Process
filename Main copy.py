...
def train_epoch(model, training_data, optimizer, pred_loss_func, opt):
    ...
    for batch in tqdm(
        ...
        # type prediction
        pred_loss, pred_num_event = Utils.type_loss(
        ...
        total_num_pred += (
            event_type.ne(Constants.PAD).sum().item() - event_time.shape[0]
        )
...
def train(
    model, training_data, validation_data, optimizer, scheduler, pred_loss_func, opt
):
    """ Start training. """
    ...
    for epoch_i in range(opt.epoch):
        ...
        print(
            "  - (Training)    loglikelihood: {ll: 8.5f}, "
        ...
        start = time.time()
        valid_event, valid_type, valid_time = eval_epoch(
            model, validation_data, pred_loss_func, opt
        )
        print(
            "  - (Testing)     loglikelihood: {ll: 8.5f}, "
            "accuracy: {type: 8.5f}, RMSE: {rmse: 8.5f}, "
            "elapse: {elapse:3.3f} min".format(
                ll=valid_event,
                type=valid_type,
        ...
        print(
            ...
            "Maximum accuracy: {pred: 8.5f}, Minimum RMSE: {rmse: 8.5f}".format(
                event=max(valid_event_losses),
                pred=max(valid_pred_losses),
                rmse=min(valid_rmse),
            )
        )
        ...
        with open(opt.log, "a") as f:
            f.write(
                "{epoch}, {ll: 8.5f}, {acc: 8.5f}, {rmse: 8.5f}\n".format(
                    epoch=epoch, ll=valid_event, acc=valid_type, rmse=valid_time
                )
            )
...
def main(opt):
    ...
    if opt.smooth > 0:
        pred_loss_func = Utils.LabelSmoothingLoss(
            opt.smooth, num_types, ignore_index=-1
        )
    else:
